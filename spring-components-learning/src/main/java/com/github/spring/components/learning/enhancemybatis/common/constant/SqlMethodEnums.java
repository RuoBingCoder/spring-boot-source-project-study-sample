package com.github.spring.components.learning.enhancemybatis.common.constant;

/**
 * @author: jianlei.shi
 * @date: 2020/12/26 4:31 下午
 * @description: SqlMethodEnums
 */
...
//    UPDATE_BY_ID("updateById", "根据ID 选择修改数据", "<script>\nUPDATE %s %s WHERE %s=#{%s} %s\n</script>"),
    UPDATE("updateBySelective", "根据 whereEntity 条件，更新记录", "<script>\nUPDATE %s %s %s\n</script>",int.class),
...
//    SELECT_BY_ALL("selectByAll", "根据ID 查询一条数据", "SELECT %s FROM %s %s"),
...
//    SELECT_LIST("selectList","根据实体类动态查询","<script>\nSELECT %s FROM %s %s\n</script>");
    private final String method;
    private final String desc;
    private final String sql;
    private final Class<?> returnType;

    /*SqlMethodEnums(String method, String desc, String sql) {
        this.method = method;
        this.desc = desc;
        this.sql = sql;
    }*/

    SqlMethodEnums(String method, String desc, String sql, Class<?> returnType) {
        this.method = method;
        this.desc = desc;
        this.sql = sql;
